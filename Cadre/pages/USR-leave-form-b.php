...
// (B) DRAW DAYS 
?>
<h3 class="mb-3">NGÀY XÁC NHẬN</h3>
<div id="leave-apply" class="zebra my-4">
  <?php for ($unix = strtotime($_POST["from"]); $unix <= strtotime($_POST["to"]); $unix += 86400) {
    ...
    <div class="d-flex align-items-center border p-2">
      <div class="flex-grow-1">
        ...
        <?php } ?>
        <?php if ($clash) { ?>
          <div class="text-danger">Bạn đã nộp đơn nghỉ phép vào ngày này.</div>
        <?php } ?>
      </div>
      <select data-date="<?= $date ?>" class="form-select w-auto" onchange="leave.total()" <?= $dayNo || $clash ? " disabled" : "" ?>>
        ...
        <?php } else { ?>
          <?php if (!$dayHalf) { ?>
            <option value="1">Tất cả ngày</option>
          <?php } ?>
  ...
  <div class="d-flex align-items-center border p-2 fw-bold">
    <div class="flex-grow-1">Tổng ngày</div>
    <div id="leave-total-days">0</div>
  </div>
</div>

<input type="button" class="btn btn-danger" value="Trở lại" onclick="cb.page(1)">
<input type="button" class="btn btn-primary" value="Xác nhận áp dụng" onclick="leave.save()">